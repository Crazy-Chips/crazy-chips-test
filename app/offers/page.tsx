  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-[#FFF8EE]">
        {/* Hero */}
        <div className="bg-[#D92B2B] py-8 px-4 text-center relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <h1 className="text-3xl sm:text-4xl text-white mb-2" style={{ fontFamily: 'var(--font-lilita)' }}>
              Current Offers
            </h1>
        ...
        <div className="max-w-xl mx-auto px-4 sm:px-6 py-8 space-y-4">
          {deals.map((deal) => (
            <div
              key={deal.code}
              className={`rounded-[18px] p-5 border-l-[4px] ${
                deal.highlight
            ...
            >
              <div className="flex items-start justify-between mb-3 gap-4">
                <div>
                  <span
                    className={`text-3xl font-[400] leading-none block mb-0.5 ${deal.highlight ? 'text-[#FFD600]' : 'text-[#D92B2B]'}`}
                    style={{ fontFamily: 'var(--font-lilita)' }}
                  >
                    {deal.discount}
                  </span>
                  <span className={`text-sm font-[700] ${deal.highlight ? 'text-white' : 'text-[#3D2200]'}`}>
                    {deal.title}
                  </span>
                </div>
              ...
              <p className={`text-xs font-[600] mb-3 leading-relaxed ${deal.highlight ? 'text-white/80' : 'text-[#8a7a6a]'}`}>
                {deal.desc}
              </p>

              <div className={`flex flex-col gap-0.5 rounded-[10px] px-3 py-2.5 mb-3 ${deal.highlight ? 'bg-white/15' : 'bg-[#FFF8EE]'}`}>
                <span
        ...
        <div className="text-center pb-16">
          <Link
            href="/menu"
            className="inline-flex items-center gap-2 bg-[#D92B2B] hover:bg-[#B01E1E] text-white font-[800] px-7 py-3 rounded-[12px] text-sm transition-all duration-200 hover:-translate-y-0.5 shadow-[0_4px_14px_rgba(217,43,43,.35)]"
          >
            Order & Save Now 🍟
          </Link>
